...
4. The full loader runs once per session and holds at or below `90%` until visible `data-critical-asset` media is ready, with a `3s` readiness timeout. Later document loads keep the short `300-500ms` choreography when cached assets are ready, but never bypass the readiness gate. The loader always removes itself and restores scrolling.
...
6. Cars and subdivisions use progressive disclosure on phones. Seasons use a single-open year accordion; departments within the open year remain fully expanded and member portraits/details are visible inline. A hash target opens the matching group. Sponsor tiers remain constantly visible without tabs or accordion controls. Inactive galleries and member portraits remain lazy.
